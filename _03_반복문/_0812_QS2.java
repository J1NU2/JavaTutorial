...
public class _0812_QS2 {
	...
	public static void main(String[] args) {
		...
		// 문자 <-> 숫자 형변환은 Integer.parseInt()를 사용하거나 Integer.valueOf()를 사용해야 한다.
		...
		for (int i=0; i<a.length(); i++) {	// 0부터 문자열 a의 마지막 인덱스까지 반복
			if (a.charAt(i) == '/') {		// charAt메소드로 떼어낸 문자가 '/'일 경우의 조건
				// Integer.parseInt() : 문자열을 파싱하여 int 객체로 바꿔준다.
		...
		// 마지막으로 추가된 vermin 문자열 result값에 추가하기 위해 해당 문자열을 파싱하여 정수형으로 변환한 뒤 result값에 더한다.
